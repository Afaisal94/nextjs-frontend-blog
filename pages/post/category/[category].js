...
const fetcher = async (url, category) => {
  const response = await axios.get(
    `https://mern-backend-blog-app.herokuapp.com/${url}/categoryname/${category}`
  );
...
export default function HomeSearch() {
  ...
  const { data, error } = useSWR(['posts', category], fetcher);
  ...
  return (
    <Layout>
      ...
      <Header title={"Blog Category"} tagline={`Posts by category : ${category}`} />
      <Container>
        <Row>
